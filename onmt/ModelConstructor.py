...
from onmt.Models import NMTModel, DoubleEncNMTModel, MeanEncoder, RNNEncoder, \
                        StdRNNDecoder, InputFeedRNNDecoder, DoubleRNNEncoder
from onmt.modules import Embeddings, ImageEncoder, CopyGenerator, \
                         TransformerEncoder, TransformerDecoder, \
                         CNNEncoder, CNNDecoder


def make_embeddings(opt, word_dict, feature_dicts, for_encoder=True, for_encoder_int=False):
    """
    ...
    elif for_encoder_int:
        embedding_dim = opt.inter_word_vec_size
    else:
...
def make_encoder(opt, embeddings,embeddings_inter=None):
    ...
    if opt.encoder_type == "transformer":
        ...
        return DoubleRNNEncoder(opt.rnn_type, opt.brnn, opt.dec_layers,
                                opt.rnn_size, opt.dropout, embeddings,embeddings_inter)
    else:
        # "rnn" or "brnn"
        print("The double encoder will be needed here")
        opt.brnn = True
...
def make_decoder(opt, embeddings):
    ...
    if opt.decoder_type == "transformer":
        ...
        return CNNDecoder(opt.dec_layers, opt.rnn_size,
                          ...
                          embeddings)
    elif opt.input_feed: #Defailt choice in decoder!!!
...
def make_base_model(model_opt, fields, gpu, checkpoint=None):
    ...
    if model_opt.model_type == "text":
        ...
        if model_opt.encoder_type=="double_encoder":
            ...
            inter_embeddings =make_embeddings(model_opt,inter_dict,
                                            inter_feature_dicts,for_encoder=False, for_encoder_int=True)
