...
class Translator(object):
    ...
    def buildTargetTokens(self, pred, src, attn, copy_vocab, inter=None):
            ...
            for i in range(len(tokens)):
                if tokens[i] == vocab.itos[onmt.IO.UNK]:
                    ...
                    try:
                        ...
                        maxIndex_new = maxIndex[0] - len(src)
                        tokens[i] = self.fields["inter"].vocab.itos[inter[maxIndex_new]]

        return tokens

    def _runTarget(self, batch, data, inter_act=False):

        if inter_act:
            src=[]
            _, src_lengths = batch.src
            src.append(onmt.IO.make_features(batch, 'src'))
            _, inter_lengths = batch.inter
            src.append(onmt.IO.make_features(batch, 'inter'))
        else:
            _, src_lengths = batch.src
        ...
        if inter_act:
            encStates, src_context, inter_hidden, inter_context = self.model.encoder(src, src_lengths,inter_lengths)
            context = torch.cat((src_context, inter_context), dim=0)
            decStates = self.model.decoder.init_decoder_state(
                src[0], context, encStates)
        else:
    ...
    def translateBatch(self, batch, dataset, inter_act=False):
        ...
        #print(src[1][:])
        if inter_act:
            encStates, src_context, enc_inter_hidden, inter_context = self.model.encoder(src, src_lengths,inter_lengths)
            context = torch.cat((src_context, inter_context), dim=0)
            decStates = self.model.decoder.init_decoder_state(
                src[0], context, encStates)
        else:
        ...
        if "tgt" in batch.__dict__:
            allGold = self._runTarget(batch, dataset, inter_act)
        else:
    ...
    def translate(self, batch, data, inter_act=False):
        #  (1) convert words to indexes
        batch_size = batch.batch_size

        #  (2) translate
        pred, predScore, attn, goldScore = self.translateBatch(batch, data,inter_act)
        ...
        if self.opt.inter:
            inter =batch.inter[0].data.index_select(1,perm)
        if self.opt.tgt:
