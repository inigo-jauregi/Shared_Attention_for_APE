...
if opt.word_vec_size != -1:
    ...
    opt.inter_word_vec_size = opt.word_vec_size
...
def eval(model, criterion, data, fields):
    ...
    for batch in valid_data:
        ...
        if opt.encoder_type=="double_encoder":
            src=[]
        ...
        if opt.encoder_type == "double_encoder":
            outputs, attn, _ = model(src,tgt,src_lengths, inter_lengths)
        else:
    ...
    def train_epoch(epoch):
        ...
        for i, batch in enumerate(train):
            ...
            if opt.encoder_type=="double_encoder":
                ...
                src = onmt.IO.make_features(batch, 'src')
            tgt = onmt.IO.make_features(batch, 'tgt')
            #For the statistics of the source
            ...
            for j in range(0, target_size-1, trunc_size):
                ...
                if opt.encoder_type=="double_encoder":
                    outputs, attn, dec_state = \
                        model(src, tgt, src_lengths,inter_lengths, dec_state)
                else:
                    outputs, attn, dec_state = \
...
def main():
    ...
    #Fields format:
    #{'inter':<torchtext.data...>, 'src':<torchtext.data...>, ...}
    fields = onmt.IO.ONMTDataset.load_fields(
        torch.load(opt.data + '.vocab.pt'))
    #Only take the fields that appear in the examples
    ...
    if opt.encoder_type=="double_encoder":
        print(' * vocabulary size. source = %d; inter = %d; target = %d' %
              (len(fields['src'].vocab), len(fields['inter'].vocab), len(fields['tgt'].vocab)))
    else:
    ...
    if opt.train_from:
        ...
        optimizer_print=checkpoint['optim'].optimizer.state_dict()
        ...
        if opt.encoder_type=="double_encoder":
            model.encoder.embeddings_src.load_pretrained_vectors(opt.pre_word_vecs_enc,
                                                                 opt.fix_word_vecs_enc)
            model.encoder.embeddings_inter.load_pretrained_vectors(opt.pre_word_vecs_enc_inter,
                                                                   opt.fix_word_vecs_enc_inter)
        else:
